...
function displayPhotographerOfThePage(photographerOfThePage, mediasOfThePage) {
  const {
    city, country, name, portrait, price, tagline,
  } = photographerOfThePage;
...
function displayMediaOfThePage(
  ...
  mediasOfThePage.forEach((galleryMedia) => {
    const gallery = document.querySelector('.gallery');
    const {
      title, likes, image, video, photographerId, id,
    } = galleryMedia;
...
GESTION DU SORT
*/
function toggleSortMenu(LastActiveSort, photographerOfThePage, mediasOfThePage) {
  let activeSort = LastActiveSort;
  ...
  likesBtn.forEach((btn) => {
    btn.addEventListener('click', (e) => {
      if (!btn.hasAttribute('data-clicked')) {
        ...
        whichMedia.likes += 1;
        const newBtn = btn;
        newBtn.innerHTML = `${whichMedia.likes} <i class="fa-solid fa-heart"></i>`;
        e.stopPropagation();
        displayPhotographerDetailsFooter(price, mediasOfThePage);
...
function lightboxModalNav(LastIndex, mediasOfThePage) {
